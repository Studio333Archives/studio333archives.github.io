...
function player(){const t=active(),sub=state.audioError||t?.pages?.[0]?.title||'STUDIO 333 ARCHIVE';return `<footer class="player ${state.audioError?'has-error':''}"><button class="p-thumb" data-player aria-label="Open queue">${t?.art?`<img src="${esc(t.art.media_url)}" alt="">`:'<span>333</span>'}</button><button class="p-toggle icon-button" data-toggle aria-label="${state.playing?'Pause':'Play'}">${icon(state.playing?'pause':'play')}</button><button class="p-title" data-player><strong>${esc(t?.title||'LISTENING SPACE')}</strong><span>${esc(sub)}</span></button><label class="p-seek-wrap" aria-label="Seek through track"><span class="p-seek-rail"><span class="p-seek-fill"></span></span><input class="p-seek" aria-label="Seek" type="range" min="0" max="${state.duration||1}" step="0.1" value="${Math.min(state.time,state.duration||0)}"></label><time>${fmt(state.time)} / ${fmt(state.duration)}</time><button class="p-prev icon-button" data-prev aria-label="Previous track">${icon('prev')}</button><button class="p-next icon-button" data-next aria-label="Next track">${icon('next')}</button><button class="p-queue icon-button" data-player aria-label="${state.playerOpen?'Close queue':'Open queue'}">${icon(state.playerOpen?'down':'up')}</button></footer><aside class="player-drawer ${state.playerOpen?'open':''}">${playerDrawer()}</aside>`}
