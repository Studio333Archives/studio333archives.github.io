...
function videoSrc(v,background=false){
  if(!v)return'';if(v.provider==='youtube'){const id=v.video_id||'';return `https://www.youtube-nocookie.com/embed/${encodeURIComponent(id)}?autoplay=1&mute=${background?1:0}&controls=${background?0:1}&loop=${background?1:0}&playlist=${encodeURIComponent(id)}&playsinline=1&modestbranding=1&rel=0`}
  ...
  return v.media_url||v.embed_url||'';
}
function videoMedia(v,background=false){if(!v)return'';const src=videoSrc(v,background);if(v.provider==='local')return `<video ${background?'autoplay muted loop':''} ${background?'':'controls'} playsinline preload="metadata" src="${esc(src)}"></video>`;return `<iframe src="${esc(src)}" title="${esc(v.title||'Studio 333 video')}" allow="autoplay; encrypted-media; picture-in-picture; fullscreen" allowfullscreen loading="lazy"></iframe>`}
...
function home(){
  ...
  return `<section class="hero">
    ...
    <div class="hero-counter"><b>${state.tracks.length}</b><span>RECORDINGS</span><b>${state.pages.length}</b><span>OBJECTS</span></div><div class="scroll-cue">SCROLL ↓</div>
  </section>
